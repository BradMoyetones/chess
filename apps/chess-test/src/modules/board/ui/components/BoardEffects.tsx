...
import type { BoardEffect } from '@/modules/board/core/ports/BoardController.port';
import { toCoords } from '@/modules/shared/utils/coordinates';
...
const DrawIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" className={cn('w-full h-full drop-shadow-md', props.className)} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M8 12h8" />
        <path d="M12 8v8" />
...
/**
 * Modular, declarative effects system.
 * Receives an array of effects from the controller — no game logic inside.
 * Adding a new effect type = just define a new BoardEffect object in the controller.
 */
export function BoardEffects({ effects, flipped }: BoardEffectsProps) {
...
// ─── Animated Square Effect ──────────────────────────────────────────────────

function AnimatedSquareEffect({ effect, flipped }: { effect: BoardEffect; flipped: boolean }) {
    ...
    const coords = toCoords(effect.square);
    if (!coords) return null;

    const squareSize = 100 / 8;
    const x = flipped ? 7 - coords.x : coords.x;
    const y = flipped ? 7 - coords.y : coords.y;
    const bgColor = VARIANT_COLORS[effect.variant] || VARIANT_COLORS.danger;
    ...
    // Determine icon based on effect type and variant
    const isLoserEffect = effect.variant === 'danger';
    const icon = (() => {
        switch (effect.type) {
            case 'checkmate':
                // For checkmate, loser gets white cross icon, winner gets black cross icon
                return isLoserEffect ? <CheckmateIconWhite /> : <CheckmateIconBlack />;
            case 'winner':
                return <WinnerIcon />;
            case 'timeout':
                return <TimeoutIcon />;
        ...
        >
            ...
            {phase === 'initial' ? (
                <>
                    ...
                    >
                        <span
                            className={cn('text-xs md:text-lg font-bold uppercase whitespace-nowrap')}
                            style={{ color: bgColor }}
                        >
                            {label}
                        </span>
                    ...
                    {/* SVG Initial State */}
                    <motion.div
                        layoutId={`icon-${effect.id}`}
                        initial={{ scale: 0, x: '-50%', y: '-50%' }}
                        animate={{ scale: 1, x: '-50%', y: '-50%' }}
                        transition={{ type: 'spring', bounce: 0.4 }}
                        className="absolute flex items-center justify-center z-20 text-white"
                        style={{
                            width: '65%',
                            height: '65%',
                            top: '50%',
                ...
                <>
                    ...
                    <motion.div
                        layoutId={`icon-${effect.id}`}
                        className="absolute flex items-center justify-center z-40 text-white"
                        initial={{ x: targetX, y: targetY }}
                        animate={{ x: targetX, y: targetY }}
                        style={{
                            top: targetTop,
